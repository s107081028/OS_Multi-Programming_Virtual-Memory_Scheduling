...
class FileSystem {
  public:
	...
	int OpenAFile(char *name){
		filePtr = Open(name);
		DEBUG(dbgFile, "OpenAFile");
		if (!filePtr) return -1;
		return 1;
	}
	int WriteFile(char *buffer, int size){
		DEBUG(dbgFile, "WriteFile");
		return (filePtr->Write(buffer, size) == 0) ? -1 : filePtr->Write(buffer, size);
	}
	int ReadFile(char *buffer, int size){
		DEBUG(dbgFile, "ReadFile");
		return (filePtr->Read(buffer, size) == 0) ? -1 : filePtr->Read(buffer, size);
	}
	int CloseFile(){
		filePtr->~OpenFile();
		filePtr=NULL;
		DEBUG(dbgFile, "CloseFile");
		if(filePtr) return -1;
		return 1;
	}
	//
