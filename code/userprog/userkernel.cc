...
void
UserProgKernel::SwapOut(int num) {
	for(int i = 1; i <= execfileNum; i++)
		if(t[i]->space->swapOut(num, t[i]->getName())) break;
}

void
UserProgKernel::SwapIn(int num) {
	currentThread->space->swapIn(num, machine->ReadRegister(BadVAddrReg));
}
